 ...
 */
contract BeliefMarket is IBiteSupplicant, ReentrancyGuard {
    ...
    function submitPosition(
        uint256 _marketId,
        bytes calldata _encryptedDirection,
        uint256 _deposit
        ...
        if (_encryptedDirection.length == 0) revert InvalidEncryptedData();
        ...
        positions[positionId] = Position({
            ...
            marketId: _marketId,
            deposit: _deposit,
            encryptedDirection: _encryptedDirection,
            direction: false,
    ...
    function resolveMarket(
        ...
        bytes[] memory encryptedArgs = new bytes[](posCount);
        for (uint256 i = 0; i < posCount; i++) {
            encryptedArgs[i] = positions[posIds[i]].encryptedDirection;
        }

        bytes[] memory plaintextArgs = new bytes[](2);
        plaintextArgs[0] = abi.encode(_marketId);
        plaintextArgs[1] = abi.encode(_outcome);
        ...
        emit MarketResolutionTriggered(_marketId, _outcome, msg.sender);
    }
